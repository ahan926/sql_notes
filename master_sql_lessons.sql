...
-- ==============================================================================
-- SQL CORE PRACTICE: INTERVIEW BLUEPRINT & PATTERNS
-- Summary of Core Intermediate Problems 1 through 9
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- PROBLEM 1: BASIC JOIN & AGGREGATION
-- Scenario: Find total sales revenue generated by each product category.
-- Tables: products (product_id, category), order_items (product_id, quantity, unit_price)
-- ------------------------------------------------------------------------------
SELECT 
    p.category, 
...
-- Key Takeaway: Wrap calculated expressions inside SUM() when grouping to avoid row-level errors.


-- ------------------------------------------------------------------------------
-- PROBLEM 2: PRE- VS. POST-AGGREGATION FILTERING (WHERE vs. HAVING)
-- Scenario: Find all customers who placed 3 or more orders in 2026.
...
-- Key Takeaway: WHERE filters individual records before grouping; HAVING filters aggregate totals.


-- ------------------------------------------------------------------------------
-- PROBLEM 3: CONDITIONAL BUCKETING (CASE WHEN)
-- Scenario: Categorize orders into High (>= $150), Medium ($50 - $149.99), and Low (< $50).
-- Table: orders (order_id, total_amount)
-- ------------------------------------------------------------------------------
SELECT 
    order_id, 
    total_amount,
...
-- Key Takeaway: Order CASE WHEN branches logically from strict to lenient to eliminate redundant AND statements.


-- ------------------------------------------------------------------------------
-- PROBLEM 4: NULL HANDLING WITH FALLBACKS
-- Scenario: Primary email preferred, fallback to secondary email, or default string if both are NULL.
-- Table: customers (customer_id, primary_email, secondary_email)
-- ------------------------------------------------------------------------------
SELECT 
    customer_id,
    COALESCE(primary_email, secondary_email, 'No Email Provided') AS contact_email
FROM customers;

-- Key Takeaway: COALESCE evaluates left-to-right and returns the first non-NULL expression.


-- ------------------------------------------------------------------------------
-- PROBLEM 5: LEFT JOIN WITH ZERO-FILLED AGGREGATIONS
-- Scenario: Calculate total spend per customer, showing 0.00 for customers with no orders.
-- Tables: customers (customer_id, customer_name), orders (order_id, customer_id, total_amount)
-- ------------------------------------------------------------------------------
SELECT 
    c.customer_name, 
...
-- Key Takeaway: Wrap COALESCE around SUM() to catch NULL aggregate outputs produced by unmatched LEFT JOIN rows.


-- ------------------------------------------------------------------------------
-- PROBLEM 6: DISTINCT COUNTING WITH PRE-FILTERING
-- Scenario: Count unique active users per country.
-- Table: users (user_id, country, status)
-- ------------------------------------------------------------------------------
SELECT 
    country, 
...
-- Key Takeaway: Filter with WHERE before grouping, and use COUNT(DISTINCT) to prevent duplicate counts.


-- ------------------------------------------------------------------------------
-- PROBLEM 7: CONDITIONAL AGGREGATION (PIVOTING ROWS TO COLUMNS)
-- Scenario: Count completed vs. cancelled orders as separate columns per customer.
...
-- Key Takeaway: SUM(CASE WHEN condition THEN 1 ELSE 0 END) converts row status flags into explicit column metrics.


-- ------------------------------------------------------------------------------
-- PROBLEM 8: FILTERING MULTIPLE AGGREGATES
-- Scenario: Customers who spent >= $500 total, but whose last order date was before 2026.
...
HAVING SUM(total_amount) >= 500 
   AND MAX(order_date) < '2026-01-01';

-- Key Takeaway: Combine multiple aggregate criteria in the HAVING clause using AND.


-- ------------------------------------------------------------------------------
-- PROBLEM 9: SELF-JOIN HIERARCHICAL COMPARISON
